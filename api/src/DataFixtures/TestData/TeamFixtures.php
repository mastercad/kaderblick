...
class TeamFixtures extends Fixture implements DependentFixtureInterface, FixtureGroupInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        foreach ($teamData as $data) {
            $name = $data['name'];
            $ageGroup = $this->getReference('age_group_' . $data['age_group'], AgeGroup::class);
            $league = $this->getReference('league_' . $data['league'], League::class);
            ...
            if ($existing) {
                // Falls Club noch nicht zugeordnet ist, hinzufügen
                if (!$existing->getClubs()->contains($club)) {
                    $existing->addClub($club);
                    $manager->persist($existing);
                }
                $this->addReference($name, $existing);
                continue;
            }
            $team = new Team();
            $team->setName($name);
