...
/**
 * @template-extends ServiceEntityRepository<CoachTeamAssignment>
 *
 * @implements OptimizedRepositoryInterface<CoachTeamAssignment>
 */
class CoachTeamAssignmentRepository extends ServiceEntityRepository implements OptimizedRepositoryInterface
{
    ...
    /**
     * @param array<int> $coachTeamAssignmentIds
     */
    public function deleteByIds(array $coachTeamAssignmentIds): void
    {
        if (0 === count($coachTeamAssignmentIds)) {
            return;
        }
        ...
        $this->createQueryBuilder('cta')
            ->delete()
            ->where('cta.id IN (:ids)')
            ->setParameter('ids', $coachTeamAssignmentIds)
            ->getQuery()
            ->execute();
    }
}
