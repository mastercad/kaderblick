...
/**
 * @template-extends ServiceEntityRepository<CoachNationalityAssignment>
 *
 * @implements OptimizedRepositoryInterface<CoachNationalityAssignment>
 */
class CoachNationalityAssignmentRepository extends ServiceEntityRepository implements OptimizedRepositoryInterface
{
    ...
    /**
     * @param array<int> $coachNationalityAssignmentIds
     */
    public function deleteByIds(array $coachNationalityAssignmentIds): void
    {
        if (0 === count($coachNationalityAssignmentIds)) {
            return;
        }

        $this->createQueryBuilder('cna')
            ->delete()
            ->where('cna.id IN (:ids)')
            ->setParameter('ids', $coachNationalityAssignmentIds)
            ->getQuery()
            ->execute();
    }
}
