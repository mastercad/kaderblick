...
//            ->setParameter('now', $now)
    ...
    /**
     * @param array<int> $playerTeamAssignmentIds
     */
    public function deleteByIds(array $playerTeamAssignmentIds): void
    {
        if (0 === count($playerTeamAssignmentIds)) {
            return;
        }

        $this->createQueryBuilder('pta')
            ->delete()
            ->where('pta.id IN (:ids)')
            ->setParameter('ids', $playerTeamAssignmentIds)
            ->getQuery()
            ->execute();
    }
}
